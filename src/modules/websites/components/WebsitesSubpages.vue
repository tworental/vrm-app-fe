<template>
    ...
    <div class="card__header">
      <div>
        <div class="card__heading uk-flex uk-flex-middle">
          ...
          />
          <h5 class="uk-margin-small-left card__title">{{ $t('websites.components.subpages.title') }}</h5>
        </div>
        <p class="card__description">
          {{ $t('websites.components.subpages.description') }}
        </p>
      </div>
      <div>
        <button
          ...
          v-e2e="'create-subpage'"
          @click="onAdd"
        >
          <core-svg width="16" :src="require('@/assets/images/icons/add.svg')" />
          <span class="--from-wide-mobile">{{ $t('websites.components.subpages.create') }}</span>
        </button>
      </div>
    </div>
      ...
      <table class="uk-table uk-table-divider table uk-margin-remove uk-text-nowrap">
        ...
        <tr>
          <th class="table__heading --filled --bordered --name-col">
            {{ $t('websites.components.subpages.table.name') }}
          </th>
          <th class="table__heading --filled --bordered uk-text-center">
            {{ $t('websites.components.subpages.table.title') }}
          </th>
          <th class="table__heading table__heading--actions --filled --bordered">
            {{ $t('websites.components.subpages.table.action') }}
          </th>
        </tr>
        </thead>
        <tbody>
        <tr class="table__row" v-for="(item) in pages" :key="item.id">
            ...
            <div class="actions">
              <template v-if="actionVisible === item.id">
                <button
                  type="button"
                  v-e2e="'edit'"
                  :uk-tooltip="'title: ' + $t('websites.components.subpages.actions.edit')"
                  class="uk-button --icon table__action-btn animated-icon"
                  @click="$router.push({ name: 'website-editor', params: { id: $route.params.id, pageId: item.id } })"
                >
                ...
                <button
                  ...
                  v-e2e="'delete'"
                  :uk-tooltip="'title: ' + $t('websites.components.subpages.actions.delete')"
        ...
        <tr class="table__row --footer">
          <td colspan="7" class="table__no-data">
            {{ $t('websites.components.subpages.noInfo') }}
          </td>
        </tr>
        </tfoot>
      </table>
  ...
  <websites-subpage-modal ref="modalRef" />
</template>
...
import WebsitesSubpageModal from '@/modules/websites/components/WebsitesSubpageModal.vue';

export default {
  components: { WebsitesSubpageModal },
  setup() {
    const i18n = useI18n();
    const uikit = useKit();
    ...
    const pages = computed(() => store.state.websites.pages.list);
    ...
    const onEdit = async (id) => {
      await store.dispatch('websites/pages/setEntity', {
    ...
    const onDelete = (id) => uikit.modals.deleteConfirm(i18n.t('websites.components.subpages.deleteConfirm'))
      .then((dialog) => {
        loading.value = id;

        return store.dispatch('websites/pages/delete', {
          websiteId: website.value.id,
          id,
        })
          .then(() => store.dispatch('websites/pages/setList', website.value.id))
          .then(() => uikit.notify('success', i18n.t('websites.components.subpages.deleteSuccess')))
          .finally(() => {
            loading.value = null;
            dialog.hide();
          });
