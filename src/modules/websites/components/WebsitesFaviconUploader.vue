<template>
  ...
  >
    <div class="wrapper" @click="fileInputRef?.$el.click()">
      <div v-show="!image">
        <div>
          <span uk-icon="icon: cloud-upload; ratio: 0.75"></span>
        </div>
      </div>
      <div v-show="!image">
        <v-field
      ...
      <div v-show="image" class="preview">
        <core-image :src="image" :fallback="require('@/assets/images/img-error.png')" />
      </div>
      <div class="overlay uk-animation-fade uk-position-cover"></div>
    </div>
    <div @click="fileInputRef?.$el.click()">
      <div class="uk-text-middle hint uk-margin-small-left">
        {{ $t('core.components.imageUploader.label') }}
        <span class="uk-link hint">{{ $t('core.components.imageUploader.select') }}</span>
      </div>
    </div>
  </div>
</template>
