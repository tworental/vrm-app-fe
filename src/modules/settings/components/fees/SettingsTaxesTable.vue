    ...
    <div class="card__header">
      <div>
        <div class="card__heading uk-flex uk-flex-middle">
          ...
          />
          <h5 class="uk-margin-small-left card__title">{{ $t('settings.views.taxes.title') }}</h5>
        </div>
        <p class="card__description">
          {{ $t('settings.views.taxes.description') }}
        </p>
      </div>
      <div>
        <button
        ...
        >
          ...
          <span class="--from-wide-mobile">{{ $t('settings.views.taxes.create') }}</span>
        </button>
      </div>
    </div>
      ...
      <table class="uk-table uk-table-divider table uk-margin-remove uk-text-nowrap">
        ...
        <tr>
          <th class="table__heading --filled --bordered --name-col">
            {{ $t('settings.views.taxes.name') }}
          </th>
          <th class="table__heading --filled --bordered uk-text-center">
            {{ $t('settings.views.taxes.tax') }}
          </th>
          <th class="table__heading --filled --bordered uk-text-center">
            {{ $t('settings.views.taxes.frequency') }}
          </th>
          <th class="table__heading --filled --bordered uk-text-center">
            {{ $t('settings.views.taxes.chargeType') }}
          </th>
          <th class="table__heading table__heading--actions --filled --bordered">
            {{ $t('settings.views.taxes.action') }}
          </th>
        </tr>
        </thead>
        <tbody>
          ...
          <td class="table__cell table__cell--actions" :class="{ '--loading': loading === item.id }">
            <div class="actions">
              <template v-if="actionVisible === item.id">
                <router-link
                  v-can:taxes.write="user"
                  :to="{ name: 'settings-taxes-overview', params: { id: item.id } }"
                  v-e2e="'edit-tax'"
...
export default {
  ...
  setup() {
    ...
    const onDelete = (id) => uikit.modals.deleteConfirm(i18n.t('settings.views.taxes.deleteConfirm'))
      .then((dialog) => {
        loading.value = id;

        return store.dispatch('settings/taxes/delete', id)
          .then(() => store.dispatch('settings/taxes/setList'))
          .then(() => uikit.notify('success', i18n.t('settings.views.taxes.deleteSuccess')))
          .finally(() => {
            loading.value = null;
            dialog.hide();
          });
