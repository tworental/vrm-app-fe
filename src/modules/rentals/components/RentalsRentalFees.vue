    ...
    <div class="card__header">
      <div>
        <div class="card__heading uk-flex uk-flex-middle">
          ...
          />
          <h5 class="card__title">{{ $t('rentals.components.rentalFees.title') }}</h5>
        </div>
        <p class="card__description">
          {{ $t('rentals.components.rentalFees.description') }}
        </p>
      </div>
      <div class="uk-flex uk-flex-top uk-flex-right search__wrapper">
        <core-form-select
          ...
          name="fee"
          :data="dict"
          :placeholder="$t('rentals.components.rentalFees.search')"
          :search="true"
        ...
        <thead>
          <tr>
            <th class="table__heading --filled --bordered --name-col">
              {{ $t('rentals.components.rentalFees.name') }}
            </th>
            <th class="table__heading --filled --bordered uk-text-center">
              {{ $t('rentals.components.rentalFees.fee') }}
            </th>
            <th class="table__heading --filled --bordered uk-text-center">
              {{ $t('rentals.components.rentalFees.frequency') }}
            </th>
            <th class="table__heading --filled --bordered uk-text-center">
              {{ $t('rentals.components.rentalFees.chargeType') }}
            </th>
            <th class="table__heading table__heading--actions --filled --bordered">
              {{ $t('rentals.components.rentalFees.action') }}
            </th>
          </tr>
        </thead>
        <tbody>
        ...
        <tfoot v-if="!fees.length">
          <tr class="table__row --footer">
            <td colspan="7" class="table__no-data">
              {{ $t('rentals.components.rentalFees.noInfo') }}
            </td>
          </tr>
        </tfoot>
      </table>
...
export default {
  ...
  setup() {
    ...
    const feeToAttach = ref(null);

    const user = computed(() => store.state.account.user);
    const limits = computed(() => store.state.core.limits);
    const fees = computed(() => store.state.rentals.fees.list);
    const isAllowed = computed(() => hasAccess(limits.value, LIMITS.APP_FEES_ENABLED, user.value, MODULES.FEES));
    const dict = computed(
      () => store.state.settings.fees.list
        .filter((fee) => !fees.value.map(({ feeId }) => feeId).includes(fee.id))
        .map((fee) => ({
          label: fee.name,
          value: fee.id,
        })),
    ...
    const onDelete = (propertyId, id) => uikit.modals.deleteConfirm(i18n.t('rentals.components.rentalFees.deleteConfirm'))
      .then((dialog) => store.dispatch('rentals/fees/delete', { propertyId, id })
        .then(() => store.dispatch('rentals/fees/setList', propertyId))
        .then(() => uikit.notify('success', i18n.t('rentals.components.rentalFees.deleteSuccess')))
        .finally(() => dialog.hide()))
    ...
    const onAttach = (propertyId, event) => {
      ...
      feeToAttach.value = event.target.value;

      store.dispatch('rentals/fees/create', { propertyId, feeId: feeToAttach.value })
        .then(() => store.dispatch('rentals/fees/setList', propertyId))
        .then(() => uikit.notify('success', i18n.t('rentals.components.rentalFees.attachSuccess')))
        .then(() => { feeToAttach.value = null; })
        .catch((error) => error && uikit.notify('danger', error.message));
    };

    onMounted(() => {
      if (isAllowed.value) {
        store.dispatch('rentals/fees/setList', route.params.id);
